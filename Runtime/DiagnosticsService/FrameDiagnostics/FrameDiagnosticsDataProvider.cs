...
namespace RealityToolkit.DevTools.DiagnosticsService.FrameDiagnostics
{
    ...
    {
        /// <inheritdoc />
        public FrameDiagnosticsDataProvider(string name, uint priority, BaseMixedRealityProfile profile, IDiagnosticsService parentService)
        ...
        public override void LateUpdate()
        {
            ...
            if (elapsedSeconds >= FrameSampleRate)
            {
                ...
                {
                    DiagnosticsService.RaiseFrameRateChanged(newCPUFrameRate, false);
                    CPUFrameRate = newCPUFrameRate;
                }

                if (newGPUFrameRate != 0 && GPUFrameRate != newGPUFrameRate)
                {
                    DiagnosticsService.RaiseFrameRateChanged(newGPUFrameRate, true);
                ...
                DiagnosticsService.RaiseMissedFramesChanged(missedFrames);
